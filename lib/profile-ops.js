...
export function manifestDeclaresProfileBundle(manifest) {
...
export function packageDeclaresProfileBundle(packageDir) {
    try {
        const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8'));
        if (!manifestDeclaresProfileBundle(manifest))
            return false;
        const declared = manifest.dsh?.bundle?.patch;
...
export function readProfile(dir) {
    ...
    const manifest = raw;
    if (typeof manifest !== 'object' || manifest === null)
        throw new Error('profile 清单格式错误');
    return {
...
export function backupProfile(dir) {
...
export function validateManifest(dir) {
    ...
    const manifest = raw;
    if (typeof manifest !== 'object' || manifest === null)
        throw new Error('写后校验失败: 清单格式错误');
    const dependencies = manifest.dependencies ?? {};
    if (typeof dependencies !== 'object' || dependencies === null)
        throw new Error('写后校验失败: 依赖列表缺失');
    const bundles = manifest.dsh?.profile?.bundles ?? [];
    if (!Array.isArray(bundles))
        throw new Error('写后校验失败: 启用列表缺失');
 ...
 */
export function writeProfileSafe(dir, manifest) {
    const pkgPath = join(dir, 'package.json');
    ...
    try {
        ...
        catch { /* 回滚失败也保留备份文件供手动恢复 */ }
        throw e;
    }
}
/** 把包加入启用列表(幂等;已在则无变化)。 */
export function addBundle(dir, packageName) {
    const current = readProfile(dir);
    if (current.bundles.includes(packageName))
        return { backup: '', changed: false };
...
export function removeBundle(dir, packageName) {
 ...
 */
export function patchEntryIds(patchText) {
    const ids = new Set();
...
export function knownCoreEntryIds(basePatchDir, webPatchDir) {
    const ids = new Set();
 ...
 */
export function patchConflictIds(pluginDir, coreIds) {
