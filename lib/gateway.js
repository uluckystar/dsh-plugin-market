...
import { CORE_BUNDLE_WHITELIST, addBundle, patchConflictIds, patchEntryIds, readProfile, removeBundle, } from "./profile-ops.js";
...
export class PluginMarketGateway extends Service {
    ...
    async install(fullName) {
        ...
        try {
            ...
            if (pkgResp.ok) {
                ...
                if (pkg.content) {
                    const parsed = JSON.parse(Buffer.from(pkg.content, 'base64').toString('utf8'));
                    const hasDsh = parsed.dsh !== undefined && (parsed.dsh.bundle !== undefined || parsed.dsh.client !== undefined);
                    if (!hasDsh) {
                        return {
    ...
    lifecycleFor(fullName, input) {
        ...
        if (this.loadedModuleNames.has(depName))
    ...
    async enable(fullName) {
        ...
        const dir = this.profileDir();
        if (!dir)
            return { ok: false, fullName: name, status: 'installed', detail: '找不到当前配置目录', restartRequired: false };
