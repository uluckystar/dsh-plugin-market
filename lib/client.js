window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				children: [
					...
					view === "browse" ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						browseState.status === "ready" ? (0, react_jsx_runtime.jsxs)("div", {
							...
							}) : browseState.result.plugins.map((p) => PluginRow(p, busyFor(p.full_name), handleInstall, handleUninstall, handleAssess, t)), allPlugins !== null && browseState.result.plugins.length < filteredList(browseState.result.category).length ? (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
					...
					view === "search" ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						searchState.status === "ready" ? (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								}), searchState.result.local.map((p) => PluginRow(p, busyFor(p.full_name), handleInstall, handleUninstall, handleAssess, t))] }) : null,
								...
								}), searchState.result.ai.map((p) => PluginRow(p, busyFor(p.full_name), handleInstall, handleUninstall, handleAssess, t))] }) : null,
					...
					view === "installed" ? (0, react_jsx_runtime.jsxs)("div", {
						...
						}) : null, installedRows.map((p) => PluginRow(p, busyFor(p.full_name), handleInstall, handleUninstall, handleAssess, t))]
					}) : null
				]
			});
		}
		...
		const zh = {
			...
			installed: "已安装",
			installedTag: "已安装",
			restartHint: "✅ 安装成功！需重启 DSH 生效（设置页无法热载，重启 dsh-web 进程后插件才会出现）。",
		...
		const en = {
			...
			restartHint: "✅ Installed! Restart DSH to activate (plugins load at startup — refresh alone is not enough).",
