...
import {
  CORE_BUNDLE_WHITELIST, addBundle, manifestDeclaresProfileBundle, packageDeclaresProfileBundle,
...
type PackageManifestFetch =
  | { readonly status: 'ok'; readonly manifest: unknown }
...
function friendlyOperationError(error: unknown, action: 'install' | 'uninstall' | 'enable' | 'disable'): string {
  ...
  if (/404|not found|repository not found|could not find/i.test(text)) return '未找到可安装版本，请确认插件地址。'
  if (/timeout|timed out|ETIMEDOUT|ECONNRESET|ENOTFOUND|EAI_AGAIN|fetch failed|network/i.test(text)) return '网络或下载失败，请检查网络后重试。'
  switch (action) {
...
export class PluginMarketGateway extends Service {
  ...
  private readonly fetchDispatcher?: any
  ...
  constructor(ctx: Context, config: PluginMarketConfig) {
    ...
    try {
      const inventory = ctx.get('pluginInventory') as {
        list(): { entries: Array<{ moduleName: string; enabled?: boolean; fiberPhase?: string | null }> }
      } | undefined
      if (inventory !== undefined) {
        for (const entry of inventory.list().entries) {
          if (entry.enabled !== false && entry.fiberPhase === 'active') this.loadedModuleNames.add(entry.moduleName)
        }
      }
  ...
  private validatedCache: Record<string, ValidationStatus> = {}
  /** 校验是否已在跑。 */
  ...
  private fetchOptions(headers: Record<string, string>, signal?: AbortSignal): RequestInit & { dispatcher?: any } {
    const init: RequestInit & { dispatcher?: any } = { headers }
    if (signal !== undefined) init.signal = signal
    if (this.fetchDispatcher !== undefined) init.dispatcher = this.fetchDispatcher
    return init
  }

  /** 拉取仓库 package.json：优先 API(token 可用时)，否则走 raw/HEAD，避免未认证 API 60/h 限流。 */
  private async fetchPackageManifest(fullName: string): Promise<PackageManifestFetch> {
    const [owner, repo] = fullName.split('/')
    if (!owner || !repo) return { status: 'missing' }
    ...
    if (this.config.githubToken !== '') {
      try {
        ...
        if (resp.ok) {
          ...
          if (body.content === undefined) return { status: 'missing' }
          try {
            return { status: 'ok', manifest: JSON.parse(Buffer.from(body.content, 'base64').toString('utf8').replace(/^\uFEFF/, '')) as unknown }
          } catch {
    ...
    try {
      ...
      try {
        return { status: 'ok', manifest: JSON.parse((await resp.text()).replace(/^\uFEFF/, '')) as unknown }
      } catch {
        return { status: 'missing' }
      }
    } catch {
  ...
  private loadValidated(): void {
    try {
      if (existsSync(this.validatedDiskPath)) {
        const disk = JSON.parse(readFileSync(this.validatedDiskPath, 'utf8')) as Record<string, ValidationStatus>
    ...
    try {
      ...
      const worker = async () => {
        while (cursor < todo.length) {
          ...
          if (done % 20 === 0) this.saveValidated()
  ...
  private activeModuleNames(): Set<string> {
    try {
      ...
      if (inventory !== undefined) {
        ...
        for (const entry of inventory.list().entries) {
          if (entry.enabled !== false && entry.fiberPhase === 'active') active.add(entry.moduleName)
        }
        return active
      }
  ...
  /** 已安装表 → 生命周期推导输入。 */
  private async lifecycleInput(): Promise<{ depByRepo: Map<string, string>; repos: Set<string>; activeModules: Set<string> }> {
    const installed = await this.installedWithSources()
   ...
   */
  private lifecycleFor(fullName: string, input: { depByRepo: Map<string, string>; repos: Set<string>; activeModules: Set<string> }): MarketPluginLifecycle {
    ...
    if (input.activeModules.has(depName)) return { status: 'enabled-active', restartRequired: false, installedName: depName }
    if (conflicts.length > 0) {
      return {
        status: 'incompatible', restartRequired: false, installedName: depName,
  ...
  private spawnRestartWrapper(): boolean {
    try {
  ...
  async enable(fullName: string): Promise<MarketToggleResult> {
    ...
    const dir = this.profileDir()
    if (!dir) return { ok: false, fullName: name, status: 'installed', detail: '找不到当前插件配置，请检查 DSH 是否正常启动。', restartRequired: false }
