...
export class PluginMarketGateway extends Service {
  ...
  private async startValidation(plugins: readonly MarketPlugin[]): Promise<void> {
    ...
    try {
      ...
      console.log(`[plugin-market] 开始校验 ${todo.length} 个插件有效性… githubToken=${this.config.githubToken !== '' ? '已配置' : '未配置'}`)
        ...
        try {
          ...
          if (resp.status === 404) {
            ...
            if (body.content) {
              ...
              this.validatedCache[p.full_name] = manifestDeclaresProfileBundle(parsed) ? 'valid' : 'invalid'
            } else {
              this.validatedCache[p.full_name] = 'invalid'
            }
          }
  ...
  async install(fullName: string): Promise<MarketInstallResult> {
    ...
    try {
      ...
      const parsed = pkg.content !== undefined
        ? JSON.parse(Buffer.from(pkg.content, 'base64').toString('utf8')) as unknown
        : null
      if (!manifestDeclaresProfileBundle(parsed)) {
