	...
	factory: (require) => {
		...
		*/
				...
				children: [
					...
					restartPendingFor !== null && !restarting ? (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)("span", {
							...
							children: lifecycleState?.canAutoRestart === false ? t("restartCap_manual") : t(`restartCap_${lifecycleState?.restartCapability ?? "pm2"}`)
						}), lifecycleState?.canAutoRestart !== false ? (0, react_jsx_runtime.jsx)("button", {
							type: "button",
		...
		const zh = {
			...
			restartCap_docker: "检测到 DSH 运行在 Docker 容器中。请确认容器配置了自动重启策略（如 restart: unless-stopped），否则退出后不会自动拉起。",
			restartCap_systemd: "检测到 DSH 由 systemd 托管。请确认服务配置了自动重启（Restart=on-failure 或 always），否则退出后不会自动拉起。",
			restartCap_manual: "当前 DSH 由手动启动，退出后不会被自动拉起，所以不提供自动重启。请手动重启 DSH 后生效。",
		...
		const en = {
			...
			restartCap_pm2: "DSH is managed by PM2, so it will come back automatically after restart.",
			restartCap_docker: "DSH runs in a Docker container. Make sure the container has a restart policy (e.g. restart: unless-stopped), otherwise it will not come back.",
			restartCap_systemd: "DSH is managed by systemd. Make sure the service has Restart=on-failure or always, otherwise it will not come back.",
			restartCap_manual: "DSH was started manually, so it cannot restart itself. Please restart DSH manually to apply changes.",
